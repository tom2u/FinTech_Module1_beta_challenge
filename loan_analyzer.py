...
# YOUR CODE HERE!
loans_average = loans_sum / loans_len
print(loans_average)
...
# HINT: Present Value = Future Value (Face Value of the Loan) / Cumulative Discount Rate Until Maturity
# YOUR CODE HERE!
# expected_sale_price / (1 + (hurdle_rate / 12)) ** holding_months
...
if (present_value >= loan_price):
    print(
        f"The present value ({present_value}) is greater than or equal to the loan price ({loan_price})")
else:
    print(
        f"The present value ({present_value}) is less than the loan price ({loan_price})")
...
present_value = calculate_present_value(
    new_loan.get("future_value"), new_loan.get("remaining_months"), annual_discount_rate)
